...
final class HomeViewController: UIViewController {
    ...
    @objc private func notificationTapped() {
        print("[OriChat][Click] Home notification tapped TODO route=notifications")
    }

    private func openWeb(entryName: String, route: WebRoute, query: String? = nil) {
        guard let url = route.finalURL(extraQuery: query) else {
            print("[OriChat][Click] \(entryName) route=\(route) invalid URL")
            return
        }
        print("[OriChat][Click] entry=\(entryName) route=\(route) finalURL=\(mask(url.absoluteString))")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if collectionView.tag == Section.featured.rawValue {
            ...
            cell.onReport = { [weak self] in
                self?.openWeb(entryName: "Home Catch Spot Report", route: .report, query: self?.reportQuery(for: item))
            }
            cell.onOpen = { [weak self] in
                self?.openWeb(entryName: "Home Catch Spot", route: .catchDetail, query: item.id)
            }
            return cell
        } else {
            ...
            cell.configure(with: item)
            cell.onReport = { [weak self] in
                self?.openWeb(entryName: "Home Angler Guide Report", route: .report, query: self?.reportQuery(for: item))
            }
            cell.onOpen = { [weak self] in
                self?.openWeb(entryName: "Home Angler Guide", route: .guideDetail, query: item.id)
            }
            return cell
        }
    }
