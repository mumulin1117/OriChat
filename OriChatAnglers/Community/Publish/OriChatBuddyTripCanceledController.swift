...
final class OriChatBuddyTripCanceledController: UIViewController {
    ...
    private func configureUI() {
        ...
        let post = GradientButton(type: .system)
        post.setTitle("Post again", for: .normal)
        post.addTarget(self, action: #selector(postAgainTapped), for: .touchUpInside)
        let stack = UIStackView(arrangedSubviews: [icon, title, subtitle, close, post])
        stack.axis = .vertical
        ...
        NSLayoutConstraint.activate([
            ...
            post.widthAnchor.constraint(equalTo: stack.widthAnchor),
            post.heightAnchor.constraint(equalToConstant: 52)
        ])
    }
