...
final class OriChatTrustAvatarPipeline {
    func load(into imageView: RemoteImageView, profile: OriChatTrustUserProfile) {
        ...
        if let avatarURL = profile.avatarURL, avatarURL.isEmpty == false {
            imageView.setImageURL(avatarURL, placeholderColor: UIColor.white.withAlphaComponent(0.08))
            return
        }
