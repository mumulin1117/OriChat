...
final class OriChatBuddyStateLedger {
    ...
    init(defaults: UserDefaults = .standard, authStore: OriaUserauthStore = .oriaCloudsyncShared) {
    ...
    private var key: String {
        let owner = authStore.oriaVerifiedanglerCurrentUserId ?? authStore.oriaVerifiedanglerCurrentUser?.oriaUserauthEmail ?? "guest"
    ...
    private func migrateLegacyAssets(in trips: [OriChatBuddyTrip]) -> [OriChatBuddyTrip] {
        trips.map { trip in
            var migrated = trip
            migrated.coverAssetName = normalizedAssetName(migrated.coverAssetName)
    ...
    private func migrate(angler: OriChatBuddyAngler) -> OriChatBuddyAngler {
        var migrated = angler
        migrated.avatarAssetName = normalizedAssetName(migrated.avatarAssetName)
    ...
    private func normalizedAssetName(_ assetName: String) -> String {
        if assetName.hasPrefix("orichat_buddy_") {
            return assetName.replacingOccurrences(of: "orichat_buddy_", with: "oriangler_buddy_")
        }
        return assetName
    }
