...
final class ProfileViewController: UIViewController {
    ...
    private func bindActions() {
        headerView.onFollowing = { [weak self] in self?.open(route: .oriaFollowerfishFollowing, entry: "Following") }
        headerView.onFollowers = { [weak self] in self?.open(route: .oriaFananglingFollowers, entry: "Followers") }
        headerView.onPosts = { [weak self] in
            guard let self else { return }
            self.render(self.viewModel.select(tab: .myPosts))
            self.open(route: .oriaProfilepicHomepage, entry: "Post count", extraQuery: self.currentState?.summary.userId)
        }
    ...
    private func handle(record: OriChatCatchProfileRecord) {
        ...
        case .view:
            open(route: .oriaPopularcatchDetail, entry: "Record view", extraQuery: record.id)
        case .review:
            open(route: .oriaBugreport, entry: "Record review", extraQuery: "tripId=\(record.id)")
        case .chat:
            open(route: .oriaReplymessagePrivate, entry: "Record chat", extraQuery: record.trip.organizer.id)
        }
    }

    private func open(route: OriaTravelrouteWebRoute, entry: String, extraQuery: String? = nil) {
        guard let url = route.oriaTravelrouteFinalURL(oriaDataanglingExtraQuery: extraQuery, authStore: authStore) else { return }
        print("[OriChat][Profile] entry=\(entry) route=\(route) oriaTravelrouteFinalURL=\(masked(url.absoluteString))")
    ...
    private func masked(_ text: String) -> String {
        ...
        if range.upperBound == suffix {
            return "\(text[..<range.upperBound])<missing>\(text[suffix...])"
        }
    ...
    @objc private func editTapped() {
        open(route: .oriaProfilepicEdit, entry: "Edit profile")
    }

    @objc private func settingsTapped() {
        open(route: .oriaMasterfishSettings, entry: "Settings")
    }

    @objc private func walletTapped() {
        open(route: .oriaVirtualcurrencyWallet, entry: "My wallet")
    }
    ...
    @objc private func signOutTapped() {
        authStore.oriaSessiontimeoutSignOut()
