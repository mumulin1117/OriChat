...
final class OriChatCatchProfileService {
    private let network: OriaApiintegratCurrentService
    private let authStore: OriaUserauthStore

    init(network: OriaApiintegratCurrentService = .oriaCloudsyncShared, authStore: OriaUserauthStore = .oriaCloudsyncShared) {
        self.network = network
        self.authStore = authStore
    }

    func fetchCurrentUser(completion: @escaping (Result<OriChatCatchProfileSummary, Error>) -> Void) {
        let userId = authStore.oriaVerifiedanglerCurrentUserId ?? authStore.oriaVerifiedanglerCurrentUser?.oriaVerifiedanglerUserId ?? OriChatBuddyFixtureForge.currentAngler.id
        let payload: [String: Any] = [
            "undertowOria": userId,
            "whirlpoolOria": 1
        ]
        network.oriaApiintegratPost(endpoint: .oriaProfilepicUserInfo, payload: payload) { [weak self] result in
    ...
    func fallbackSummary() -> OriChatCatchProfileSummary {
        ...
        return OriChatCatchProfileSummary(
            userId: authStore.oriaVerifiedanglerCurrentUserId ?? user?.oriaVerifiedanglerUserId ?? OriChatBuddyFixtureForge.currentAngler.id,
            name: user?.oriaBiotextNickname.nonEmptyValue ?? "OriChat Angler",
