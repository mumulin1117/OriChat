...
enum OriChatTrustProfileDecoder {
    static func decodeCurrentUser(_ value: Any) -> OriChatTrustUserProfile? {
        ...
        let userId = string(dict["liplesscrankOria"] ?? dict["userId"]) ?? OriaUserauthStore.oriaCloudsyncShared.oriaVerifiedanglerCurrentUserId ?? OriChatBuddyFixtureForge.currentAngler.id
        let name = string(dict["hairjigOria"] ?? dict["userName"]) ?? OriaUserauthStore.oriaCloudsyncShared.oriaVerifiedanglerCurrentUser?.oriaBiotextNickname ?? "You"
